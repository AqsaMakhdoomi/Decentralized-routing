# driver = futility.index(min(futility))
...
##THESE ARE dc
fd_s = [427, 944, 994, 438, 474, 448, 847, 481, 841, 1097, 890, 681, 652, 494, 309, 1046, 943, 556, 405, 688, 159, 521,
        129, 1079, 800, 1216, 1024, 328, 863, 689, 623, 524, 454, 1158, 802, 273, 591, 1218, 702, 1000, 878, 773, 527,
        4, 1150, 366, 387, 401, 190, 1143, 1228, 568, 713, 496, 850, 369, 593, 1211, 1208, 597, 18, 340, 845, 641, 585,
        253, 956, 980, 50, 925, 939, 268, 1098, 861, 632, 58, 821, 1128, 56, 1229, 516, 859, 755, 1244, 1025, 633, 158,
        517, 571, 714, 489, 500, 154, 1215,
        246]  # [0, 350, 682, 334, 762, 647, 1074, 663, 229, 865, 1142, 380, 418, 1197, 819, 909, 644, 995, 67, 723, 266, 650, 524, 94, 1093, 781, 777, 714, 1158, 552, 1240, 859, 886, 276, 809, 1036, 998, 997, 106, 546, 509, 858, 175, 1054, 822, 921, 1216, 125, 238, 1204, 457, 917, 1059, 400, 704, 322, 501, 780, 43, 1025, 372, 1003, 631, 1182, 582, 579, 247, 1176, 876, 883, 23, 710, 811, 940, 1041, 659, 844, 393, 512, 1, 494, 1232, 48, 994, 1157, 960, 407, 963, 543, 474, 701, 912, 1002, 689, 470, 890]
fd_s = [427, 944, 994, 438, 863, 689, 623, 524, 190, 1143, 1228, 568, 585, 253, 956, 980]
...
for azm in range(0, iterations):
    for cd in range(1, n_i):  # see how 1000 drivers earn over 100 iterations
        ...
        for ak in prev_up:
            ...
            res_G = res_G.reshape(576, 576)
            np.save(
                'C:\\Users\\Dell\\Downloads\\Route recomm env predicted data\\NY2km1folder\\Ny_data_opt_veh_cou\\26\\' + str(
        ...
        if cd == 1:
            ...
            #         drivers=[]
            drivers.append(futility.index(min(futility)))  # random.randint(0, n - 1))
        for hm in drivers:
            ...
            if driver_i == 0:  # in first iteration every user calculates optimal route on initial graph
                ...
                if len(drivers) > 1:
                    gutility = []
                    for im in drivers:
                        gutility.append(futility[im])
                    print("gutility", gutility)
                    print("futility", futility)
                    driver = futility.index(
                        min(gutility))  # drivers[random.randint(0,                       len(drivers) - 1)]  # random.randint(0,n-1)]  # random.randint(0,n)  #generate a driver randomly to update routes
                ...
                for id in cln[driver]:  # range(0,30):#cln[driver]:
                    ...
                    if com_reqs:
                        ...
                        print("futility_new[id] > futility[id]", futility_new[id] > futility[id])
                        #   print("futility_new[id], futility[id]", futility_new[id], futility[id])
